...
# Путь к CA-бандлу с сертификатами НУЦ Минцифры (создаётся install_nuc_cert.py)
_CA_BUNDLE = str(Path(__file__).resolve().parent.parent / "ca_bundle.pem")
if not Path(_CA_BUNDLE).exists():
    _CA_BUNDLE = True  # fallback на certifi
...
class TInvestClient:
    def __init__(
        self, token: str, request_pause_sec: float = 0.2, verify_ssl: bool = True
    ) -> None:
        ...
        # Если verify_ssl=True, используем кастомный CA-бандл (с сертификатами НУЦ)
        # Если verify_ssl=False — отключаем проверку
        self._session.verify = _CA_BUNDLE if verify_ssl else verify_ssl
